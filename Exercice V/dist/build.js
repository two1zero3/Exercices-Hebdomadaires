...
class Circle {
    constructor(x, y, rayon, context, index) {
        this.x = x;
        this.y = y;
        this.origin = { x: x, y: y };
        this.target = { x: x, y: y };
    ...
    shaderColor() {
        ...
        this.color = `rgb(${color0.r * 255},${color0.g * 255},${color0.b * 255})`;
    }
    lengthGLSL(x, y) {
...
var hauteur = 1000;
var gridXY = 20;
var gridX = largeur / gridXY;
var gridY = hauteur / gridXY;
...
function setup() {
    ...
    for (let j = 0; j < hauteur; j += 20) {
        for (let i = 0; i < largeur; i += 20) {
            //get index that is goes up by one for every new circle
            let circle = new Circle(i, j, 10, context, index);
